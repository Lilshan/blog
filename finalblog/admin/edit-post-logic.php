...
if(isset($_POST['submit'])) {
    ...
    // set is_featured to 0 if is was unchecked
    ...
    if(!$title) {
        ...
        if($thumbnail['name']) {
            ...
            // rename image
            $time = time(); // make each image name upload enique using current timestamp
            $thumbnail_name = $title . $thumbnail['name'];
            $thumbnail_tmp_name = $thumbnail['tmp_name'];
            $thumbnail_destination_path = '../images/' . $thumbnail_name;

            // make sure file is an image
            $allowed_files = ['PNG','png', 'JPEG','jpeg' , 'JPG' , 'jpg'];
